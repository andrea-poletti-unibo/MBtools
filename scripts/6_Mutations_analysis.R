...
m_ALL_fl <- left_join(m_AF_fl, m_DP_fl) %>% left_join(m_GQ_fl) %>% left_join(m_NGT_fl) %>% left_join(m_RG)
...
for( SAMP in list_h5_files){
  ...
  SNPs_VAFhist_plot
  ggsave(paste0("plots/SNP_analysis/",samplename, "SNP_VAFhist_plot.png" ), width = 12, height = 12)
